...
import com.savinghumanity.fx.FXFactory;
...
public class GameEngine {
	...
	private FXFactory effectFactory;
		...
		for(int i = 0 ; i < playerTankList.size() ; i++){
			for(int j = 0; j < playerTankList.get(i).getTankBonuses().size() ; j++){
				...
				if((curTime - tanksBonus.getStartingTime()) / 1000000000.0 > tanksBonus.getDurationTime()){
					...
					playerTankList.get(i).deeffect(tanksBonus);
	...
	public void createEffect(Effect e){
		effectList.add(e);
		allObjects.add(e);
	...
	public void singleModeVictoryCheck(Stage primaryStage){
		if((enemyTankList.isEmpty() || playerTankList.isEmpty()) && currentLevel < 3){
			if(!playerTankList.isEmpty()){
				GameApplication.alertLevelFinished(currentLevel);
				startGame("data/map/Level" + (currentLevel+1) + ".txt", 1);
			}
			else{
				GameApplication.alertPlayerDie();
				currentLevel = 4;
	...
	public void buttonPressed(String keyPressed) {
		...
		for(int i = 0; i < playerTankList.size() ; i++){
			if(playerTankList.get(i).isConfused()){
				playerTankList.get(i).setSpeedX(-playerTankList.get(i).getSpeedX());
				playerTankList.get(i).setSpeedY(-playerTankList.get(i).getSpeedY());
			}
		}
				...
				if(allObjects.get(i).collisionCheck(allObjects.get(j))){
					if(obj1 instanceof Tank && obj2 instanceof Tank){
						...
						if(obj1 instanceof EnemyTank){
							((EnemyTank)obj1).setDistanceLeft(0.0f); // Reset AI distance left before new decision
						}
						...
						if(obj1 != ((Bullet)(obj2)).getOwner()){ // A bullet cannot hit its owner
							if(!((obj1 instanceof EnemyTank) && (((Bullet)(obj2)).getOwner() instanceof  EnemyTank))) // Friednly Fire is off for enemy tanks
								if((obj1 instanceof PlayerTank && !((PlayerTank)(obj1)).isInvincible()) || obj1 instanceof EnemyTank){
							...
							((Bullet)(obj2)).getOwner().setBullet(null); // Destroy the bullet
							
							if(((Tank)obj1).getHealth() <= 0){
								Effect boomEffect = effectFactory.createBoomEffect2(obj1.getPosX(), obj1.getPosY(), null, (short)15, 1, -43, -70);
								createEffect(boomEffect);
								...
								Effect boomEffect = effectFactory.createBoomEffect1(obj1.getPosX(), obj1.getPosY(), null, (short)5, 1, -43, -70);
								createEffect(boomEffect);
							}
						}
	...
	public ArrayList<PlayerTank> getPlayerTankList() {
	...
	private void generateBonus(float posX, float posY) {
		...
		if(random < 0.1f)
			...
			bonus = new RandomDestroyer(posX, posY,true,1,System.nanoTime());
